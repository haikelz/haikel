...
const ListNotes = ({ notes }: NotesArticlesProps) => {
  return (
    <div className="grid w-full grid-cols-1 grid-rows-1 gap-6 sm:grid-cols-2">
      {notes.map((note) => (
        <div
          className="overflow-hidden border-[2.5px] border-black bg-antiflashwhite transition-all duration-200 hover:shadow-light active:scale-95 dark:border-white dark:bg-raisinblack dark:hover:shadow-dark"
          key={note.slug}
        >
          <div className="h-full px-6 pb-6 pt-3">
            ...
            <div className="mt-3 flex gap-2 font-medium">
              {note.tags.map((tag) => (
                <Link
                  className="cursor-pointer bg-celedongreen px-1.5 text-sm text-white dark:bg-lightgray dark:text-slate-900"
                  key={tag}
                  href={`/tags/${tag}`}
                >
                  {tag}
